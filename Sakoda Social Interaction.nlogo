...
let tuple [1 0 -1]    
    ...
    while [count turtles with [color = 0] > 0] [
      let in_att one-of tuple
      let out_att one-of tuple
      let output? false
    ask turtles with [groupid = i2] [
      set color c
      set ingroup in_att
      set outgroup out_att
      set label groupid
      if output? = false [output-type "Group " output-type groupid output-type " " 
        output-type "In-Group: " output-type ingroup output-type "; "
        output-type "Out-Group: " output-print outgroup
        set output? true]
      ]
    set color_list fput c color_list
    while [member? c color_list = true] [
      set c one-of base-colors
      ]
    set i2 i2 + 1
    ]
